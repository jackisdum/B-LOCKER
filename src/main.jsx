...
function BLocker() {
      ...
      timerRef.current = setInterval(() => {
        setTimeLeft(t => {
          if (t <= 1) { 
            ...
            setScreen("hurdle"); 
            // NATIVE: Resume blocking when session ends
  ...
  function tryExtend() {
    if (hurdleInput.trim() === requiredPhrase) {
      ...
      // NATIVE: Extend unlock for another 5m
      BockerNative.setUnlockState({ state: true });
              ...
              {apps.map((app, i) => (
                <div key={app.id} className="row-hover" onClick={() => openApp(app)}
                  ...
                  <button onClick={e => toggleBlock(app.id, e)}
                    ...
                    <div style={{ position: "absolute", width: 20, height: 20, borderRadius: 10, background: "white", top: 3, left: app.blocked ? 21 : 3, transition: "left 0.2s", boxShadow: "0 1px 3px rgba(0,0,0,0.10)" }} />
                  </button>
                </div>
              ))}
            </div>
